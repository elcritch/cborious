...
suite "CBOR basics":
  ...
  test "tags: timestamps":
    # Tag 0: RFC3339 date/time string
    block:
      var s = CborStream.init()
      s.packTimestampString("2013-03-21T20:04:00Z")
      check s.data == "\xc0\x74" & "2013-03-21T20:04:00Z"
      s.setPosition(0)
      # Unpack as string should ignore tag
      let dt = unpack(s, string)
      check dt == "2013-03-21T20:04:00Z"

    # Tag 1: epoch seconds (example from RFC 7049)
    block:
      var s = CborStream.init()
      s.packTimestamp(1363896240'i64)
