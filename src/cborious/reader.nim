...
proc readUintArg(src: openArray[byte], pos: var int, ai: uint8): uint64 =
  ...
  if ai <= 23'u8:
    ...
    if pos + 1 > src.len: raiseCbor(ceEndOfBuffer, "need 1 byte")
    let v = uint64(src[pos])
    inc pos
    return v
  elif ai == 25'u8:
    if pos + 2 > src.len: raiseCbor(ceEndOfBuffer, "need 2 bytes")
    let v = (uint64(src[pos]) shl 8) or uint64(src[pos+1])
    pos += 2
    return v
  elif ai == 26'u8:
    if pos + 4 > src.len: raiseCbor(ceEndOfBuffer, "need 4 bytes")
    let v = (uint64(src[pos]) shl 24) or (uint64(src[pos+1]) shl 16) or (uint64(src[pos+2]) shl 8) or uint64(src[pos+3])
    pos += 4
    return v
  elif ai == 27'u8:
    if pos + 8 > src.len: raiseCbor(ceEndOfBuffer, "need 8 bytes")
    ...
    raiseCbor(ceInvalidArg, "invalid additional info for uint argument")
...
proc decodeInt64*(src: openArray[byte], pos0: int = 0, consumed: var int): int64 =
  ...
  var pos = pos0
  if pos >= src.len: raiseCbor(ceEndOfBuffer, "no bytes to read")
  let b0 = src[pos]
  inc pos
  ...
  of mtNegative:
    let u = readUintArg(src, pos, ai)
    if u == high(uint64):
      raiseCbor(ceOverflow, "negative int overflows int64")
    let v = -1'i64 - int64(u)
    consumed = pos - pos0
    return v
  else:
    raiseCbor(ceInvalidHeader, "not an integer header")
...
proc decodeBool*(src: openArray[byte], pos0: int = 0, consumed: var int): bool =
  ...
  let major = CborMajor((b0 shr 5) and 0x7)
  let ai = uint8(b0 and 0x1f)
  if major != mtSimple: raiseCbor(ceInvalidHeader, "not a simple value header")
  case ai
  of 20'u8: consumed = pos - pos0; false
  of 21'u8: consumed = pos - pos0; true
  else: raiseCbor(ceInvalidArg, "not a bool simple value")
